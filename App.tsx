...
import { User, Theme, SystemConfig } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Load System Config (Mock implementation using LocalStorage for now, in production this comes from DB)
    const savedSysConfig = localStorage.getItem('olivia_system_config');
    if (savedSysConfig) {
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('olivia_god_mode'); 
    setSession(null);
    setProfile(null);
